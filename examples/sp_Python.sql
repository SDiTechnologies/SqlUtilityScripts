...
GO
CREATE PROCEDURE generate_rental_py_model
...
-- Run the following T-SQL statement in Azure Data Studio to create a table called dbo.rental_py_models which is used to store the model.
USE TutorialDB;
...
-- Save the model to the table as a binary object, with the model name linear_model.
DECLARE @model VARBINARY(MAX);
EXECUTE generate_rental_py_model @model OUTPUT;

INSERT INTO rental_py_models
    (model_name, model)
...
-- Create a stored procedure py_predict_rentalcount that makes predictions using the trained model and a set of new data. Run the T-SQL below in Azure Data Studio.
DROP PROCEDURE IF EXISTS py_predict_rentalcount;
GO
CREATE PROCEDURE py_predict_rentalcount
    (@model varchar(100))
AS
BEGIN
    DECLARE @py_model varbinary(max) = (select model
    from rental_py_models
